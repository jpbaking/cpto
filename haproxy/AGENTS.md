...
TCP frontend for the CPTO stack. The only service with published host ports. Routes inbound HTTP (`:3128`) and SOCKS (`:1080`) connections from the host to tinyproxy and dante, which are reachable at the `openvpn` hostname because they share its network namespace.
...
- `depends_on: [openvpn, tinyproxy, dante]` is set at compose level

## Work Guidance

- Backend addresses (`openvpn:3128`, `openvpn:1080`) must stay in sync with what tinyproxy/dante actually bind
